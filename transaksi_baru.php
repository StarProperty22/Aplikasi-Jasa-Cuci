...
if (empty($_SESSION['id_user'])) {
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        // Cek apakah sudah ada yang memesan
        $check_sql = mysqli_query($koneksi, "SELECT * FROM transaksi WHERE no_nota = '$no_nota'");
        if (mysqli_num_rows($check_sql) > 0) {
            echo '<script>alert("No. Nota sudah ada, tidak bisa memesan lagi.");</script>';
        } else {
            $sql = mysqli_query($koneksi, "INSERT INTO transaksi(no_nota, jenis, nama, bayar, kembali, total, tanggal, tanggal_pemesanan, id_user) VALUES('$no_nota', '$jenis', '$nama', '$bayar', '$kembali', '$total', NOW(), NOW(), '$id_user')");
...
<form method="post" action="" class="form-horizontal" role="form">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-3">
            <input ```php
            type="number" class="form-control" id="bayar" name="bayar" placeholder="Isi dengan angka" required>
        </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        <label for="tanggal_pemesanan" class="col-sm-2 control-label">Tanggal Pemesanan</label>
        <div class="col-sm-3">
